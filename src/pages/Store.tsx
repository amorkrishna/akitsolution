...
export default function Store() {
  ...
  return (
    <div className={`min-h-screen ${bg} ${textPrimary} transition-colors duration-300 overflow-x-hidden`}>
      ...
      {/* QUICK LEAD CAPTURE — Free consultation form */}
      <section className={`border-t ${isDark ? "border-white/[0.06]" : "border-gray-100"} relative overflow-hidden`}>
        <div className={`absolute inset-0 ${isDark ? "bg-gradient-to-br from-violet-600/[0.06] via-transparent to-emerald-500/[0.04]" : "bg-gradient-to-br from-violet-50 via-white to-emerald-50/40"}`} />
        <div className={`absolute -top-20 -right-20 h-72 w-72 rounded-full blur-3xl ${isDark ? "bg-violet-600/15" : "bg-violet-300/30"}`} />
        <div className={`absolute -bottom-20 -left-20 h-72 w-72 rounded-full blur-3xl ${isDark ? "bg-emerald-500/10" : "bg-emerald-200/30"}`} />

        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 relative">
          <div className={`rounded-3xl border-2 backdrop-blur-xl overflow-hidden ${isDark ? "bg-white/[0.03] border-white/10" : "bg-white border-gray-200 shadow-2xl shadow-violet-200/30"}`}>
            <div className="grid md:grid-cols-5 gap-0">
              {/* Left: Pitch */}
              <div className={`md:col-span-2 p-6 sm:p-8 md:p-10 relative ${isDark ? "bg-gradient-to-br from-violet-600/15 to-indigo-600/10" : "bg-gradient-to-br from-violet-600 to-indigo-600 text-white"}`}>
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-4 ${isDark ? "bg-white/10 text-white" : "bg-white/20 text-white"}`}>
                  <Sparkles className="h-3 w-3" /> {lang === "bn" ? "ফ্রি কনসাল্টেশন" : "Free Consultation"}
                </span>
                <h3 className={`text-2xl sm:text-3xl md:text-4xl font-black font-display leading-tight mb-3 ${isDark ? "text-white" : "text-white"}`}>
                  {lang === "bn" ? "একদম " : "Get a "}
                  <span className={isDark ? "text-gradient" : "underline decoration-amber-300 decoration-4 underline-offset-4"}>{lang === "bn" ? "ফ্রি কোটেশন" : "free quote"}</span>
                  {lang === "bn" ? " নিন" : " in minutes"}
                </h3>
                <p className={`text-sm sm:text-base mb-5 ${isDark ? "text-white/70" : "text-white/90"}`}>
                  {lang === "bn"
                    ? "নাম ও ফোন দিন — আমাদের এক্সপার্ট ১৫ মিনিটের মধ্যে কল করবে।"
                    : "Drop your name & phone — our expert will call you back within 15 minutes."}
                </p>
                <ul className={`space-y-2 text-xs sm:text-sm ${isDark ? "text-white/80" : "text-white/95"}`}>
                  {[
                    lang === "bn" ? "১০০% ফ্রি সাইট ভিজিট" : "100% Free site visit",
                    lang === "bn" ? "অথরাইজড ডিলার প্রাইস" : "Authorized dealer pricing",
                    lang === "bn" ? "কোনো অগ্রিম পেমেন্ট নয়" : "No upfront payment",
                  ].map((b) => (
                    <li key={b} className="flex items-center gap-2">
                      <CheckCircle className={`h-4 w-4 ${isDark ? "text-emerald-400" : "text-emerald-300"}`} />
                      {b}
                    </li>
                  ))}
                </ul>
              ...
              {/* Right: Form */}
              <form
                className="md:col-span-3 p-6 sm:p-8 md:p-10 space-y-4"
                onSubmit={(e) => { e.preventDefault(); submitLead.mutate(); }}
              >
                <div>
                  <Label className={`text-xs font-semibold mb-1.5 inline-block ${textPrimary}`}>
                    {lang === "bn" ? "আপনার নাম" : "Your Name"} <span className="text-rose-500">*</span>
                  </Label>
                  <Input
                    value={leadForm.name}
                    onChange={(e) => setLeadForm({ ...leadForm, name: e.target.value })}
                    placeholder={lang === "bn" ? "যেমন: আবদুল করিম" : "e.g. John Smith"}
                    maxLength={100}
                    required
                    className={`h-11 ${isDark ? "bg-white/[0.04] border-white/10 text-white placeholder:text-white/30" : "bg-white border-gray-200"}`}
                  />
                </div>
                <div>
                  <Label className={`text-xs font-semibold mb-1.5 inline-block ${textPrimary}`}>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      type="tel"
                      required
                      className={`h-11 pl-9 ${isDark ? "bg-white/[0.04] border-white/10 text-white placeholder:text-white/30" : "bg-white border-gray-200"}`}
                    />
                  </div>
                </div>
                <div>
                  <Label className={`text-xs font-semibold mb-1.5 inline-block ${textPrimary}`}>
                    {lang === "bn" ? "কোন সেবায় আগ্রহী?" : "Service Interest"}
                  </Label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {[
                      { key: "CCTV Installation", icon: Camera, bn: "সিসিটিভি", en: "CCTV" },
                      { key: "Attendance Device", icon: CheckCheck, bn: "অ্যাটেনডেন্স", en: "Attendance" },
                      { key: "Networking", icon: Wifi, bn: "নেটওয়ার্ক", en: "Networking" },
                      { key: "Servicing & Repair", icon: Wrench, bn: "সার্ভিসিং", en: "Servicing" },
                      { key: "IT Equipment", icon: Monitor, bn: "IT যন্ত্রপাতি", en: "IT Equipment" },
                      { key: "Other", icon: Sparkles, bn: "অন্যান্য", en: "Other" },
                    ].map(({ key, icon: Icon, bn, en }) => {
                      const active = leadForm.service_type === key;
                      return (
                        <button
                          key={key}
                          type="button"
                          onClick={() => setLeadForm({ ...leadForm, service_type: key })}
                          className={`group h-11 rounded-lg border-2 px-2 text-xs font-semibold inline-flex items-center justify-center gap-1.5 transition-all ${active
                            ? "bg-gradient-to-r from-violet-600 to-indigo-500 border-transparent text-white shadow-lg shadow-violet-500/30"
                            : isDark
                              ? "bg-white/[0.03] border-white/10 text-white/70 hover:border-violet-500/40 hover:text-white"
                ...
                <Button
                  ...
                  className="w-full h-12 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-500 hover:from-violet-500 hover:to-indigo-400 text-white font-bold text-sm shadow-xl shadow-violet-500/30 border-0"
                >
                  {submitLead.isPending
                    ? (lang === "bn" ? "জমা দেওয়া হচ্ছে..." : "Submitting...")
                    : (
                      <>
                ...
                <p className={`text-[10px] text-center ${textMuted}`}>
                  {lang === "bn"
                    ? "🔒 আপনার তথ্য ১০০% সুরক্ষিত। স্প্যাম নয়।"
                    : "🔒 Your details are 100% safe. No spam, ever."}
                </p>
              </form>
            </div>
          </div>
