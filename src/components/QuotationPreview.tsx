  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative" }}>
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "3px", background: "linear-gradient(to right, #0d9488, #1e40af)", borderRadius: "2px" }} />
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", paddingTop: "12px", marginBottom: "16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div>
              ...
              <div style={{ marginTop: "6px", fontSize: "9px", color: "#6b7280", lineHeight: 1.6, whiteSpace: "pre-line" }}>
          ...
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            ...
            <div style={{ marginTop: "4px", fontSize: "10px", color: "#6b7280" }}>
              <p style={{ margin: "2px 0" }}>Date: <span style={{ fontWeight: 500, color: "#374151" }}>{new Date(quotation.issue_date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</span></p>
              {quotation.valid_until && <p style={{ margin: "2px 0" }}>Valid Until: <span style={{ fontWeight: 500, color: "#374151" }}>{new Date(quotation.valid_until).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</span></p>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: "20px", backgroundColor: "#ffffff", borderRadius: "8px", padding: "12px 14px", border: "1px solid #e5e7eb" }}>
        <p style={{ fontSize: "9px", fontWeight: 700, textTransform: "uppercase", color: "#9ca3af", letterSpacing: "0.1em", marginBottom: "4px" }}>Quote To</p>
        <p style={{ fontWeight: 600, color: "#111827", fontSize: "14px", margin: 0 }}>{quotation.clients?.name || "Walk-in Customer"}</p>
        {quotation.clients?.address && <p style={{ fontSize: "12px", color: "#4b5563", marginTop: "2px" }}>{quotation.clients.address}</p>}
      </div>

      {/* Items Table */}
      <table style={{ width: "100%", marginBottom: "16px", borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ backgroundColor: "#ffffff", color: "#111827", borderBottom: "2px solid #0d9488" }}>
            ...
            <th style={{ textAlign: "right", padding: "8px 8px", fontSize: "10px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", width: "80px" }}>Unit Price</th>
        ...
        <tbody>
          {quotation.items?.map((item: any, idx: number) => {
            ...
            const cleanDesc = item.description?.replace(/^\[(Service|Product)\]\s*/, "") || item.description;
            ...
            return (
              <tr key={idx} style={{ borderBottom: "1px solid #e5e7eb" }}>
                ...
                <td style={{ padding: "8px 8px", fontSize: "12px", color: "#1f2937", whiteSpace: "nowrap", fontFamily: "Arial, Helvetica, sans-serif", fontKerning: "none", textRendering: "geometricPrecision" }}>
                  <span style={{ whiteSpace: "nowrap", display: "inline-block" }}>{cleanDesc}</span>
                </td>
                <td style={{ padding: "8px 6px", textAlign: "center" }}>
                  <span style={{
                    display: "inline-block", padding: "1px 6px", borderRadius: "9999px", fontSize: "8px", fontWeight: 700, textTransform: "uppercase",
          ...
          {(!quotation.items || quotation.items.length === 0) && (
            <tr><td colSpan={6} style={{ padding: "20px", textAlign: "center", color: "#9ca3af", fontSize: "12px" }}>No line items</td></tr>
          )}
        </tbody>
      </table>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "16px" }}>
        <div style={{ width: "240px" }}>
          <div style={{ fontSize: "12px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", color: "#6b7280", marginBottom: "4px" }}><span>Subtotal</span><span style={{ color: "#374151" }}>৳{Number(quotation.subtotal).toLocaleString()}</span></div>
            <div style={{ display: "flex", justifyContent: "space-between", color: "#6b7280", marginBottom: "4px" }}><span>Tax ({quotation.tax_rate}%)</span><span style={{ color: "#374151" }}>৳{Number(quotation.tax_amount).toLocaleString()}</span></div>
          </div>
          <div style={{ marginTop: "6px", paddingTop: "6px", borderTop: "2px solid #0d9488" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontWeight: 700, color: "#111827", fontSize: "15px" }}>Total</span>
              <span style={{ fontWeight: 800, color: "#0d9488", fontSize: "17px" }}>৳{Number(quotation.total).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {hasBankInfo && (
        <div style={{ marginBottom: "16px", backgroundColor: "#ffffff", borderRadius: "8px", padding: "12px 14px", border: "1px solid #e5e7eb" }}>
          <p style={{ fontSize: "9px", fontWeight: 700, textTransform: "uppercase", color: "#0d9488", letterSpacing: "0.1em", marginBottom: "6px" }}>Payment Information</p>
      ...
      {quotation.notes && (
        <div style={{ marginBottom: "16px", padding: "10px 12px", backgroundColor: "#ffffff", borderRadius: "8px", border: "1px solid #e5e7eb" }}>
          <p style={{ fontSize: "9px", fontWeight: 700, textTransform: "uppercase", color: "#f59e0b", letterSpacing: "0.1em", marginBottom: "3px" }}>Terms & Conditions</p>
          <p style={{ fontSize: "12px", color: "#374151", margin: 0, whiteSpace: "pre-wrap" }}>{quotation.notes}</p>
        </div>
      )}

      {/* Validity Notice */}
      {quotation.valid_until && (
        <div style={{ marginBottom: "16px", padding: "8px 12px", backgroundColor: "#fef3c7", borderRadius: "8px", border: "1px solid #fcd34d", textAlign: "center" }}>
          <p style={{ fontSize: "10px", fontWeight: 600, color: "#92400e", margin: 0 }}>
            ⏳ This quotation is valid until {new Date(quotation.valid_until).toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" })}
          </p>
        </div>
      )}
      ...
      <div style={{ marginTop: "auto", paddingTop: "24px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "24px", alignItems: "end" }}>
          ...
          <div style={{ textAlign: "center", paddingBottom: "4px" }}>
            <img 
              src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=Verification:${quotation.quotation_number}`} 
              alt="QR Code" 
