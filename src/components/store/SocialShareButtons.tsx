...
export function SocialShareButtons({ productName, productPrice, productUrl, className = "", compact = false }: SocialShareButtonsProps) {
  const url = productUrl || window.location.href;
  const text = `🔥 ${productName} — মাত্র ৳${productPrice.toLocaleString()}! AK IT Solution থেকে অর্ডার করুন।`;
  ...
  const shareLinks = [
    ...
    },
    {
      ...
      onClick: () => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}&quote=${encodeURIComponent(text)}`, "_blank"),
    },
    {
      name: "Twitter",
      icon: () => (
      ...
      <div className={`flex items-center gap-1.5 ${className}`}>
        {shareLinks.map(link => (
          <button
            ...
            className={`${link.color} rounded-full p-2 transition-all hover:scale-110`}
    ...
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      ...
      {shareLinks.map(link => (
        <Button
          key={link.name}
          size="sm"
          className={`${link.color} gap-1.5 text-xs h-8`}
          onClick={link.onClick}
        >
          <link.icon />
          {link.name}
